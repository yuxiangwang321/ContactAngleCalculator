# ContactAngleCalculator cac-cg-20241205.tcl
# dropletMolNumDensity unit number/A^3
# length unit: A
#command example：cac all.psf all.dcd xz 900 1000 20 {resname SUB} {resname TIP3} 3 0.0324 0.1 3 0.5 0.5
#command example：cac 10.xyz  10.xyz  xz 0 0 1 {resname SUB} {resname TIP3}  1 9.0  0.1  3  1.5  1.2

set tcl_precision 7

proc cac {file_1 file_2 type firstFrame lastFrame step substrateSelection dropletAtomRange atomNumberInMol \
...
puts $logfile "Command: cac $file_1 $file_2 $type $firstFrame $lastFrame $step {$substrateSelection} {$dropletAtomRange}\
          $atomNumberInMol $dropletMolNumDensity $glRatio $sphereFactor $thicknessFactor $sliceFactor"
puts $caFile "  "
puts $caFile "********************** file: $file_1 $file_2 **********************"
puts $caFile "Command: cac $file_1 $file_2 $type $firstFrame $lastFrame $step {$substrateSelection} {$dropletAtomRange}\
          $atomNumberInMol $dropletMolNumDensity $glRatio $sphereFactor $thicknessFactor $sliceFactor"
...
#         wetting on XZ plane
	if {[string equal -nocase $type xz]} {
		...
		set substrateArea [atomselect top "$substrateSelection and x>$xl and x<$xh and z>$zl and z<$zh" frame $i]
	...
	# step_4: calculate the contact angle
		...
		set zl [lindex $dropletAtomRemainSize 0 2]
		set zh [lindex $dropletAtomRemainSize 1 2]
		set substrateArea [atomselect top "$substrateSelection and y>$yl and y<$yh and z>$zl and z<$zh" frame $i]
		set substrateSize [measure minmax $substrateArea]
	...
	# step_4: calculate the contact angle
		...
		set yl [lindex $dropletAtomRemainSize 0 1]
		set yh [lindex $dropletAtomRemainSize 1 1]
		set substrateArea [atomselect top "$substrateSelection and x>$xl and x<$xh and y>$yl and y<$yh" frame $i]
		set substrateSize [measure minmax $substrateArea]
